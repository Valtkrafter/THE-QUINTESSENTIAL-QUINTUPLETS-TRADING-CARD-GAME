...
export const SleeveStep: React.FC<SleeveStepProps> = ({ card, onFinish }) => {
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-between p-4 sm:p-6 select-none overflow-hidden">
      ...
      {/* Main Card Saver Stage */}
      <div className="relative flex-1 w-full max-w-2xl flex items-center justify-center my-3">
        {/* Semi-Rigid Outer Holder Shell */}
        <div className="relative w-[260px] sm:w-[300px] aspect-[63/95] rounded-xl border-2 border-cyan-400/40 bg-gradient-to-b from-cyan-950/20 via-white/5 to-black/40 shadow-2xl backdrop-blur-sm p-3 pt-6 flex flex-col items-center justify-end overflow-hidden">
          {/* Card Saver Lip Flap */}
          ...
          <AnimatePresence>
            {phase !== 'sliding_in' && (
              <motion.div
                ...
                className="absolute top-10 left-4 sm:left-6 z-40 w-48 sm:w-52 p-3.5 rounded-lg bg-[#fef08a] text-zinc-900 font-sans shadow-2xl border border-yellow-300"
                style={{
                  boxShadow: '0 15px 25px -5px rgba(0,0,0,0.5), 0 0 10px rgba(254,240,138,0.3)',
                }}
              >
                {/* Subtle Post-it Fold Corner */}
                <div className="absolute top-0 right-0 w-4 h-4 bg-yellow-400/60 border-l border-b border-yellow-500/40 rounded-bl" />

                <div className="text-[10px] font-mono font-black text-zinc-800 uppercase tracking-wider mb-1.5 pb-1 border-b border-zinc-800/20">
                  TQQ RESTORATION LOG
                </div>
      ...
      {/* Bottom Re-Submission Handshake Controls */}
      <div className="w-full max-w-md z-10 flex flex-col items-center gap-2.5">
        {phase === 'ready' ? (
          <motion.div
