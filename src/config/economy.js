...
// ==========================================
export const RARITY_BASE_VALUES = {
    C: 15,
    UC: 50,
    R: 200,
    SR: 900,
...
export const GRADE_TIER_CONFIG = {
    POOR_1_3: {
 ...
 */
export function calculateRawCardValue(rarity, finish) {
 ...
 */
export function calculateGradedCardValue(rawCardValue, tier) {
 ...
 */
export function calculateCardMarketValue(card) {
 ...
 */
export function calculateGradingFee(card, hasRaihaSupport = false) {
    const rawValue = calculateRawCardValue(card.rarity, card.finish);
 ...
 */
export function calculateDustYield(card, hasMaruoSupport = false) {
    const rawValue = calculateRawCardValue(card.rarity, card.finish);
 ...
 */
export function rollFinish() {
    const roll = randomFloat() * 100.0;
    let cumulative = 0.0;
    const finishes = ['signed', 'gold_etched', 'rainbow', 'sparkle', 'holo', 'raw'];
 ...
 */
export function pickRandomCardDef(rarity) {
 ...
 */
export function rollRarityFromTable(dropTable) {
    const roll = randomFloat() * 100.0;
    let cumulative = 0.0;
    const rarities = ['MR', 'SEC', 'UR', 'SR', 'R', 'UC', 'C'];
 ...
 */
export function rollPityRarity(dropTable, targetPity) {
    const pool = targetPity === 'UR'
        ? ['UR', 'SEC', 'MR']
        : ['SR', 'UR', 'SEC', 'MR'];
    let totalWeight = 0;
    for (const r of pool) {
 ...
 */
export function rollPackDrops(packId, pityCounters) {
...
// ==========================================
function generateSubgrades(tier, numericGrade) {
    if (tier === 'BLACK_LABEL') {
        return {
            centering: 10.0,
            surface: 10.0,
...
function singleGradeRoll(activeTools) {
    ...
    if (hasLaser) {
        ...
        const lowerTierKeys = ['POOR_1_3', 'USED_4_6', 'CRISP_7_8', 'MINT_9'];
 ...
 */
export function rollGrading(_card, tools = []) {
...
// ==========================================
export const NAKANO_SISTERS = ['ichika', 'nino', 'miku', 'yotsuba', 'itsuki'];
/**
 * Analyzes binder page synergies and yields.
 */
export function analyzeBinderPage(page, cardMap) {
    let totalMarketValue = 0;
    const sisterIdsPresent = new Set();
    const sisterCountMap = {};
    let totalSlottedSisters = 0;
    let supportCharId = null;
    for (const slot of page.slots) {
        ...
        if (slot.slotIndex < 5) {
            if (NAKANO_SISTERS.includes(card.characterId)) {
                const sId = card.characterId;
            ...
            // Support slot
            supportCharId = card.characterId;
        }
    }
    // Check Synergies:
 ...
 */
export function calculateAccruedIdleEarnings(report, elapsedMinutes, maxOfflineHours = 24) {
