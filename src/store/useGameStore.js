...
import { analyzeBinderPage, calculateAccruedIdleEarnings, calculateDustYield, calculateGradingFee, CONSUMABLE_TOOLS, PACKS_CONFIG, rollGrading, rollPackDrops, } from '../config/economy.js';
const DEFAULT_BINDER_SLOTS = [
...
export const useGameStore = create()(persist((set, get) => ({
    ...
    getTestSheetCooldownRemaining: () => {
        ...
        const effectiveCooldown = report.supportCharacterId === 'takeda'
    ...
    openPack: (packId) => {
        ...
        if (state.yen < packConfig.costYen) {
            throw new Error(`Insufficient Yen. Required: ${packConfig.costYen} ¥, Available: ${state.yen} ¥`);
        }
        // 3. Roll drops
    ...
    gradeCard: (cardInstanceId, toolOverrides) => {
        const state = get();
    ...
    dustCard: (cardInstanceId) => {
        ...
        if (card.slottedBinder) {
            const newSlots = state.binder.slots.map((s) => s.cardInstanceId === cardInstanceId ? { ...s, cardInstanceId: null } : s);
    ...
    buyTool: (toolId, quantity = 1) => {
        ...
        if (state.stardust < totalCost) {
            throw new Error(`Insufficient Stardust. Required: ${totalCost}, Available: ${state.stardust}`);
        }
        set({
    ...
    equipTool: (toolId) => {
        const state = get();
        if (state.equippedTools.includes(toolId))
            return;
    ...
    slotBinderCard: (slotIndex, cardInstanceId) => {
        const state = get();
        ...
        if (cardInstanceId === null) {
            ...
            const updatedSlots = state.binder.slots.map((s) => s.slotIndex === slotIndex ? { ...s, cardInstanceId: null } : s);
            const updatedInventory = state.inventory.map((c) => c.id === currentSlottedId ? { ...c, slottedBinder: undefined } : c);
            set({
        ...
        if (cardDef.characterRole !== targetRole) {
            throw new Error(`Role mismatch for slot ${slotIndex}: Card '${cardDef.name}' is role '${cardDef.characterRole}', but slot requires '${targetRole}'.`);
        }
        // If card was already in another slot, unslot it there
        const previousSlot = state.binder.slots.find((s) => s.cardInstanceId === cardInstanceId);
